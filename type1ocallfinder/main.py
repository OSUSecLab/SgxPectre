...
def handleaddresses():
	...
	for addr in info.wrfsbaselist:
		info.b.hook(addr, passf, length=5)

	'''
	# sgx_ocalloc
	info.b.hook_symbol("sgx_ocalloc", Ocalloc())

	info.b.hook_symbol("memcopy", Memcopy())

	info.b.hook_symbol("memmove", Memmove())
	'''

	info.b.hook(0x101af, passf, length = 2)
	info.b.hook(0x7108, passf, length = 5)
	info.b.hook(0x7114, passf, length = 5)
	info.b.hook(0x11077, passf, length = 5)
	info.b.hook(0x11060, passf, length = 5)
	info.b.hook(0x110a1, passf, length = 5)
	info.b.hook(0x110c0, passf, length = 5)
	info.b.hook(0x1103e, passf, length = 5)
	info.b.hook(0x15570, strnlenf, length = 1)
	info.b.hook(0x11031, passf, length = 5)
	info.b.hook(0x1155e, passf, length = 5)
...
def capstoneparse():
	...
	with open(sys.argv[1], 'rb') as f:
		if info.picflag == 1:
			seekstart = info.codeoffset - 0x400000
			f.seek(seekstart, 1)
			info.code = f.read()
	info.insns.extend(info.b.arch.capstone.disasm(info.code, info.codeoffset))
...
def findgadget():
	...
	if info.picflag == 0:
		info.state.mem[info.state.regs.gs +0x30].uint64_t = 0x7e000000
		info.state.mem[info.state.regs.gs +0x40].uint64_t = 0x7e000000
	...
	while True:
		...
		#print info.insnsmap[info.state.addr].mnemonic


		if info.firstocallmet == 1:
			#print "info.firstocallmet == 1"
		...
		if info.state.addr == 0x4049b8:
			print "stop at 0x4049b8"
			continue

		'''


		#if info.state.addr == 0x404b81:
		#	print "stop at 0x404b81"
		#	continue
		...
		if info.state.addr == 0x4032a3:
			print "stop at 0x4032a3"
			continue
		...
		if info.state.addr == 0x15229:
			lastrbp = info.state.regs.rbp
			#print lastrbp

		if info.state.addr == 0x152a0:
			info.state.regs.rbp = lastrbp
			#print info.state.regs.rsp
			#print info.state.regs.rbp
			#exit()
		...
		# hook
		if info.state.addr == 0x15bf1:
			info.state.regs.rip = 0x15caa

		if info.state.addr == 0x15cc4:
			info.state.regs.rip = 0x15e51


		'''
		info.b.hook(0x404b7c, passf, length=5)



		if info.state.addr == 0x4049af:
			print "stop at 0x4049af"
			continue
		if info.state.addr == 0x404ad7:
			print "print at 0x404ad7"
			print info.state.regs.rcx
		'''
		...
		# if enclu instruction
			...
			# make rax,rbx,rcx,rdx,rsi,rdi,r8,r9,r10,r11,r12,r13,r14,r15 symbolic
			makeregistersymbolic("ocall_");
			info.state.regs.rax = 0
			info.state.regs.edi = -2

			#info.state.regs.rsi = 0


			info.succs = [info.state]
			#continue

			if info.state.addr == 0x151ea:
				exit()
		...
		if info.b.factory.successors(info.state, num_inst=1).unconstrained_successors:
			for succ in info.b.factory.successors(info.state, num_inst=1).unconstrained_successors:
				empty = info.emptystate.copy()
				empty.se.add(succ.regs.rsp <= initial_rsp)
				empty.se.add(succ.regs.rsp >= 0)
				empty.se.add(initial_rsp >= 0)
				empty.se.add(initial_rsp - succ.regs.rsp < 0xffffffff)
				#print empty.se.satisfiable()
				#print empty.se.eval(succ.regs.rsp)
				#print empty.se.eval(initial_rsp)
